...
Reconfiguring a device -- assigning a real name/function to a currently-unused Unity input/output, or (the specific real case that prompted this) figuring out why one of this coach's Unity X270D board's dimming-capable outputs behaved as a plain on/off latch instead of a dimmer, in both the OneControl app and this project -- requires *writing* PIDs, not just reading them. This project decoded how that works (2026-08-21, decompiled LippertConnect source, cross-checked against Lippert's own official `OneControl Configurator Guide`, CCD-0001830), and has since confirmed the full write path against real hardware for one specific PID (161, see "PID Writes" under Protocol Reference below) via a manual, `--confirm`-gated diagnostic tool -- not yet a general or production-integrated write capability (see `TODO.md` for what's still deferred: `manage-devices` UI, PID 4/5 rename support, any automatic/production write path):
...
**Second goal (assigning a name to a currently-unused port) hit a real disambiguation problem, now substantially (not fully) solved (2026-08-22).** This coach's stable-key fallback's `(PRODUCT_ID, product_instance)` turned out to be a shared generic default across the entire physical Unity module, not per-port -- useless for picking out one specific unconfigured port among many. `DEVICE_ID`'s `device_instance` field (decoded since day one, never previously used) turned out to be the real answer: a sequential, gapless counter scoped per DEVICE_TYPE across the whole module. Confirmed with two independent real-hardware cross-checks against this coach's own wiring inventory, one of which was independently corroborated a second way (a candidate device's real fuse-rating PID value matched a physically-distinct output's known amperage rating exactly). This produced a high-confidence (but not yet physically confirmed) candidate mapping for this coach's specific unused DIMM/LATCH output, plus several other previously-unidentifiable ports on this same board. A new manual `--confirm`-gated tool (`src/tools/relay_blip.py`) exists to convert this from "high confidence" to "confirmed" by briefly energizing a candidate relay while the user watches a multimeter -- not yet run. Full detail, including what remains genuinely unresolved (a separate "Configurable Inputs" bank has no matching visible CAN device at all), in "device_instance -- Disambiguating Devices That Share the Fallback Stable Key" under Protocol Reference below.
...
| `src/tools/probe_common.py` | Shared bus-setup/address-claim/request-response plumbing (2026-08-21), extracted once both `pid_probe.py` and `pid_write.py` needed it. |
...
| `src/tools/relay_blip.py` | Manual, `--confirm`-gated relay ON/OFF CLI (2026-08-22). Sends a real COMMAND directly to a raw address over REMOTE_CONTROL, bypassing `command_gate.py` (target has no `device_class` to gate on). For physically confirming a candidate device identification. Not yet run. |
...
**Candidate identification for this coach's board (2026-08-22, real addresses from this coach, position-in-sequence only -- NOT yet confirmed by direct physical test):**

| device_instance | Type | Address (at capture time) | Identity |
|---|---|---|---|
| 3 | LATCHING_RELAY_TYPE_2 | `0x11` | DIMM/LATCH output 7 |
| 4 | LATCHING_RELAY_TYPE_2 | `0xE8` | DIMM/LATCH output 8 |
| 5 | LATCHING_RELAY_TYPE_2 | `0x12` | Water Heater (Elec or Gas -- pair not individually resolved) |
| 6 | LATCHING_RELAY_TYPE_2 | `0xF3` | Water Heater (Gas or Elec) |
| 8 | LATCHING_RELAY_TYPE_2 | `0xD4` | Fuel Pump -- **cross-validated**, see above |
| 5 | TANK_SENSOR | `0xE5` | Tank position 5 (Holding) |
| 6 | TANK_SENSOR | `0x16` | Tank position 6 (Holding) |
| 7 | TANK_SENSOR | `0xE7` | Tank position 7 (LP/Fuel) |
| 8 | TANK_SENSOR | `0x69` | Tank position 8 (LP/Fuel) |

Addresses are volatile (see "Address (In)Stability" below) -- valid only as of the capture they came from, must be re-resolved live before any write.
...
Reconfiguration-relevant PIDs (2026-08-21, decompiled LippertConnect source -- see "PID Reconfiguration" design decision above for the full research): 4 = `IDS_CAN_FUNCTION_NAME` (UINT16), 5 = `IDS_CAN_FUNCTION_INSTANCE` (UINT8) -- **confirmed supported** on this hardware family via `--list-pids` (both present in the real 97-PID set for Kitchen Island Light and Kitchen Pendants Light), though a write has not yet been attempted. 161 = `SIMULATE_ON_OFF_STYLE_LIGHT` -- **confirmed supported, confirmed as the real root cause, and confirmed fixed**: writing `0` (6-byte `UINT48` width) resolved the Kitchen Island Light latching-instead-of-dimming problem end-to-end, verified both on the wire and physically (see "PID Writes" below). 183 = `DEVICE_TYPE`, 451 = `LOAD_TYPE` are **confirmed NOT supported** on this Unity X270 hardware (see above) -- kept as named constants in `pid_client.py` for citation/reference only.
...
**Value width must match the PID's full declared `Formatter` type, not a read's truncated width.** The first real write attempt (PID 161, 1-byte value -- matching what a *read* of PID 161 returns) was rejected with `RESPONSE.BAD_REQUEST` (2), not a session/auth error (`SESSION_NOT_OPEN`=14, `KEY_NOT_CORRECT`=13) -- the DIAGNOSTIC handshake itself completed cleanly, so the rejection was specifically about the write payload's shape. Retrying with the full declared width (`UINT48` = 6 bytes per the decompiled catalog, giving an 8-byte total payload -- 2-byte PID + 6-byte value, exactly one CAN frame) succeeded. The takeaway: reads may arrive DLC-truncated (leading zero bytes omitted, per `parse_pid_reply()`'s existing docstring), but a write must supply the value at its nominal type width or the device rejects it outright.

**Write error-reply shape (BAD_REQUEST case) is shorter than a read's confirmed error shape:** the malformed-width write got back a single raw byte (`02`, matching `RESPONSE.BAD_REQUEST`) -- not the 5-byte `[0x0000][echoed pid(2)][response(1)]` shape confirmed for read errors above. Only this one error case has been observed for writes; other write error codes may have other shapes, unconfirmed.
