...
**Version:** 5.0 | **Updated:** 2026-08-26
...
- Battery voltage is not published — it's PID-based (request/response), not broadcast, and this project is currently passive-only for it (no read request ever sent). Deferred, low priority.
- PID battery voltage reads have not been validated against this coach's real traffic yet (no PID traffic appeared in the first capture at all). Everything else in v1's decode scope (DEVICE_ID structure, relay/motor status, dimmable light status, tank sensor status, the TEA session handshake) is confirmed against a real capture from this coach.
