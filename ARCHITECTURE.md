...
Motor status was never exposed this way — it briefly had its own read-only, non-standard-service-name D-Bus exposure (`motor_status_service.py`), confirmed working exactly as designed on real hardware 2026-08-24 (correctly invisible in the main Device List, no writable path of any kind), then removed the same day per explicit user decision ("pretty useless" — invisible in the main GUI by design, so the only real value was dbus-spy/MQTT/Node-RED visibility, which nobody had an active need for) — see "Motor Status Support — Removed" under D-Bus Service Layer below for the full history and how to reinstate it.
