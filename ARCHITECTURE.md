...
**Version:** 3.8 | **Updated:** 2026-08-24
...
`/MotorState` (raw OutputState int), `/PositionPercent`, `/CurrentAmps`, `/FaultLatch`, `/Dtc`, `/CustomName` (writeable). No other writable path exists on this service -- intentional, see the "No Motor Commands" boundary under Design Decisions above. `MotorStatusService.update()` batches all 6 paths into one `ItemsChanged` signal (2026-08-24, same fix/reasoning as `TankService` above) -- the highest-value case for this fix, since motor devices broadcast at ~333ms while moving, previously up to 6 separate D-Bus round-trips per update.
