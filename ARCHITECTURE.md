...
- Real GitHub-based auto-update (`PackageManager.py`'s `GitHubDownload`/`updateGitHubVersion`) needs: a **public** repo (plain `wget`, no auth), a `version` file starting with `v` (already the convention here), and a `gitHubInfo` file (`user:branch`). It checks `raw.githubusercontent.com/<user>/<repo>/<branch>/version` for the version string and downloads `github.com/<user>/<repo>/archive/<branch>.tar.gz` on mismatch -- ordinary GitHub archive URLs, no Releases/tags required, "latest" is just a branch-naming convention SetupHelper's own author uses, not a platform feature. Deliberately not set up yet -- the repo (`github.com/jsalbre/venus-onecontrol-can`) is currently private, and this needs a public repo. Until then, manual tarball sync + `setup install auto` (see README.md's Installation section) is the deployment path.
