...
| `ext/velib_python` (git submodule, not vendored source) | Victron's own `VeDbusService` reference implementation (MIT). `tank_service.py`/`switch_service.py`/`publisher.py` import `vedbus` directly from it. **Not this project's code** -- see README.md's Dependencies section for the clone/init requirement (`git clone --recurse-submodules`, or `git submodule update --init` after the fact); missing it silently produces a deploy tarball that fails to import on the Cerbo. |
