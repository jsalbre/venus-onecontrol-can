...
**Version:** 4.5 | **Updated:** 2026-08-26
...
**`ConfigManager` caches its parsed config in memory (2026-08-24), keyed on the config file's `(inode, mtime_ns, size)`.** Before this, `is_exposed()` and every other read (`find_device()`, `get_devices()`, `commands_enabled_for()`, etc.) did a full file-open + exclusive `flock` + `json.load()` + O(n) scan on *every single call* — and `routing.route_device_id()` calls `is_exposed()` on every DEVICE_ID broadcast, before its `already_created` short-circuit, so even a device whose service was created hours ago re-triggered a full disk read on every ~1-3Hz re-broadcast. Confirmed as the dominant driver of `publisher.py` using 11-12% CPU on real hardware (vs. 3-4% for the next-heaviest process) — 20-90+ full read cycles per second, sustained, on this coach's 20-31-device bus, for data that essentially never changes at runtime. The cache key deliberately uses `st_ino`, not mtime alone: `_atomic_write()` always replaces the file via a fresh tempfile + `os.replace()`, so the inode changes on *every* write regardless of filesystem mtime resolution (undocumented for the Cerbo's `/data`) — this makes cross-process external writes (`manage-devices`/`manage-system` editing `config.json` while `publisher.py` keeps running) reliably detected, preserving the already-established live-reconfiguration property (see "PID Reconfiguration" below) without a restart. This process's own writes additionally update the cache immediately (`ConfigManager._write_locked()`), rather than waiting on the next `read()`'s stat check. `DiscoveryLog` (`discovered_devices.json`) got the identical treatment, plus a real adjacent bug fix: its writes weren't atomic (no tempfile/`os.replace`) despite being written from two processes (`publisher.py`'s `record()` and `manage-devices`' `prune_configured()`) — both classes now share one `_atomic_write_json()` helper.
...
Motor status was never exposed this way — it briefly had its own read-only, non-standard-service-name D-Bus exposure (`motor_status_service.py`), confirmed working exactly as designed on real hardware 2026-08-24 (correctly invisible in the main Device List, no writable path of any kind), then removed the same day — "pretty useless": invisible in the main GUI by design, so the only real value was dbus-spy/MQTT/Node-RED visibility, which nobody had an active need for — see "Motor Status Support — Removed" under D-Bus Service Layer below for the full history and how to reinstate it.
...
Reconfiguring a device -- assigning a real name/function to a currently-unused Unity input/output, or (the specific real case that prompted this) figuring out why one of this coach's Unity X270D board's dimming-capable outputs behaved as a plain on/off latch instead of a dimmer, in both the OneControl app and this project -- requires *writing* PIDs, not just reading them. This project decoded how that works (2026-08-21, decompiled LippertConnect source, cross-checked against Lippert's own official `OneControl Configurator Guide`, CCD-0001830), and has since confirmed the full write path against real hardware for one specific PID (161, see "PID Writes" under Protocol Reference below) via a manual, `--confirm`-gated diagnostic tool. `manage-system` (project root, 2026-08-22) is the resulting interactive tool -- distinct from `manage-devices`, which only ever touches this bridge's own D-Bus config -- built once the dimming/latching setting (PID 161) and target disambiguation (`device_instance`) were proven real, with identity writes (PID 4/5) assumed to work the same way. That assumption was wrong in a specific way: its first real run (2026-08-23) tried to rename an unconfigured port and both PID 4 and PID 5 writes failed -- root-caused to the same universal value-width issue described in "PID Writes" below, not anything specific to identity fields, and fixed the next day. **The fix is now confirmed end-to-end on real hardware (2026-08-24):** DIMM/LATCH output 7 (`device_instance=3`, addr `0x11`) was renamed to "Front Cap Light" (`FUNCTION_NAME=169`) via `manage-system`, both PID 4/5 writes verified `PASS`, the physical relay test behaved as expected, the device was added to `config.json`, and the OneControl phone app itself now shows and controls it as "Front Cap Light" -- this closes out the original "assign a name to an unused port" goal that started this whole reconfiguration effort. **Deliberately never wired into `command_gate.py`/`publisher.py` or any other production path, by permanent design decision, not a "not yet"** -- I have ruled out ever allowing PID writes to be triggered from Venus OS/D-Bus; `manage-system` stays a manual, deliberately-run tool, matching `pid_write.py`/`relay_blip.py` precedent:
...
- No real capture can validate any of this before implementation -- the phone app can't reconfigure at all, only the physical touchscreen can, and this installation doesn't have one.
...
### PID 161 Live Read (dimmable_light D-Bus Presentation, 2026-08-24)

Every PID read/write up to this point was exclusively a manual-tool thing (`pid_probe.py`, `pid_write.py`, `manage-system`) -- `publisher.py` had never sent an active REQUEST and waited for a matching RESPONSE at all, only ever passively decoded broadcasts. This changed to solve a real gap: a `dimmable_light` configured (via PID 161, `SIMULATE_ON_OFF_STYLE_LIGHT`) to behave as a plain on/off switch -- a genuine, deliberate configuration choice per the "PID Reconfiguration" section above, not a bug -- still always rendered as a dimmer in the Venus OS GUI, since `SwitchService`'s `Settings/Type`/`Dimming` path were fixed by `device_class` alone with no awareness of PID 161's actual value.
...
Not yet exercised on real hardware -- `output_type_for()`'s new parameter has unit coverage (pure function), but the async read/create-delay machinery in `publisher.py` has none, consistent with the rest of this file's dbus/gi-dependent code (no `tests/test_publisher.py` exists at all -- see the Testability note under Source Files).
...
| `manage-system` (project root) | Interactive tool to reconfigure the Unity module itself (PID writes) -- distinct from `manage-devices`, which only touches this bridge's own D-Bus config. Configure a port (identity + any applicable known setting, e.g. PID 161's dimming/latching behavior), unconfigure a port, or back up every port's current settings to `samples/`. Opens with a mandatory typed-acknowledgment warning every run. See "PID Reconfiguration" design decision above. **Confirmed on real hardware (2026-08-24):** renamed DIMM/LATCH output 7 to "Front Cap Light" end-to-end (identity write, physical test, `config.json`, live in the OneControl app). Unconditionally opens the real CAN interface (`SocketCanBus`) before any flow runs -- including the `config.json`-only parts -- so it cannot be exercised off the Cerbo, unlike `manage-devices`. |
...
| `ext/velib_python` (vendored, not a submodule -- see "Vendored, Not a Submodule" above) | Victron's own `VeDbusService` reference implementation (MIT), pinned at commit `17bbcd4c632d3eda484cde611dc78bf8c2ba469f`. `tank_service.py`/`switch_service.py`/`publisher.py` import `vedbus` directly from it. **Not this project's code**, but tracked directly in git (not a submodule) so it survives a GitHub-archive-based update intact. |
...
|---|---|---|---|---|
| 3 | LATCHING_RELAY_TYPE_2 | `0x11` | DIMM/LATCH output 7 -- **renamed "Front Cap Light"** (2026-08-24, `manage-system`, `FUNCTION_NAME=169`) | **Confirmed and reconfigured** -- physically identified via `relay_blip.py` (2026-08-22), then renamed and confirmed live/controllable in the OneControl app (2026-08-24); no longer in the unconfigured pool |
...
**Confirmed real finding, root cause of the Kitchen Island Light dimming problem (2026-08-21), this coach:** cross-referencing the 97 PIDs discovered via `--list-pids` (see above) against the full PID catalog in `assembly_0080_Sharp.decompiled.cs` (`new PID(<num>, "<NAME>", ...)`, 449 named entries) identified **PID 161 = `SIMULATE_ON_OFF_STYLE_LIGHT`** (`Formatter.UINT48`, session requirement `2` = DIAGNOSTIC per the PID class's 4th constructor argument) -- a name that matches the reported symptom ("configure a dimming capable output as latching") almost exactly. Reading it against both real lights confirmed the hypothesis directly:
...
**End-to-end real-hardware confirmation (2026-08-21), this coach:** `src/tools/pid_write.py` (new, `--confirm`-gated, never wired into any production path) opened a DIAGNOSTIC session against Kitchen Island Light (`function_name=38,function_instance=0`), wrote PID 161 = 0 (6-byte `UINT48` width), got `RESPONSE.SUCCESS`, and a read-back confirmed the new value. Confirmed the fix physically: Kitchen Island Light dims correctly now, and the OneControl phone app itself renders it as a dimmer after an app restart (the app reads this same PID to decide how to display the device). This closes out the Kitchen Island Light investigation and is the first real confirmation that this project can successfully reconfigure a device, not just read one.
...
- `/SwitchableOutput/0/Settings/ShowUIControl` (writeable, default `1`, added 2026-08-24) -- confirmed against Victron's own dbus wiki (github.com/victronenergy/venus/wiki/dbus#switch, fetched directly, not assumed): a bitmask, not a plain enum -- `0`=hidden everywhere, `1`=always shown (bit 0 set overrides bits 1/2), `2`=local UIs only (GX/MFD/WASM over LAN), `4`=remote UIs only (VRM remote console/switch pane). Matches Node-RED's own virtual-switch UI naming exactly ("Off"/"Always"/"Only Local"/"Only on VRM" -- the feature request that prompted adding this). Persisted via `ConfigManager.get_show_ui_control()`/`set_show_ui_control()`, editable from `manage-devices` (both at add time and via "Manage existing devices"). Does nothing on this project's own side beyond persisting the value, same as `Settings/Group`.
...
- GitHub-based auto-update is set up (2026-08-25): `PackageManager.py`'s `GitHubDownload`/`updateGitHubVersion` (confirmed by reading `kwindrem/SetupHelper`'s source directly, not assumed) needs a **public** repo (plain `wget`, no auth -- the repo name must exactly equal the package name, `venus-onecontrol-can`, which it already does), a `version` file whose entire (whitespace-stripped) content is used directly as the version string, and a `gitHubInfo` file read as `fd.readline().strip().split(':')` -- a single line, `gitHubUser:gitHubBranch`, e.g. `jsalbre:main`. It checks `raw.githubusercontent.com/<user>/<repo>/<branch>/version` for the version string and downloads `github.com/<user>/<repo>/archive/<branch>.tar.gz` on mismatch -- ordinary GitHub archive URLs, no Releases/tags required, "latest" is just a branch-naming convention some SetupHelper packages use for their default branch, not a platform requirement (this project's default branch is `main`, which works the same way). The repo (`github.com/jsalbre/venus-onecontrol-can`) is now public. A fresh first install still downloads the same public archive directly and runs `setup install auto` by hand (see README.md's Installation section) -- SetupHelper needs the package present at all before it can check for updates -- GitHub-based checking only takes over for updates after that.

- **A package that's already installed doesn't pick up a new `gitHubInfo` file automatically.** Confirmed directly from `PackageManager.py`'s source: `gitHubInfo` is only ever read inside the `add` action (`AddRemoveQueue`'s `action == 'add'` branch) -- an already-registered package's `GitHubUser`/`GitHubBranch` live in persistent D-Bus Settings (`/Settings/PackageManager/<n>/GitHubUser`/`GitHubBranch`) that, per the source's own comment, "will not be changed" by anything short of editing them directly. This package was auto-discovered and added by `AddStoredPackages()` long before `gitHubInfo` existed, so adding the file later did nothing -- the fields had to be set once, manually, through PackageManager's own edit screen (which is exactly what those two fields are there for -- the source notes user/branch "can be edited by the GUI" -- and doing so triggers an immediate version re-check).

- **A real incident (2026-08-25) drove two further fixes, both required for GitHub-based auto-update to actually be safe to use, not just theoretically wired up:** the first real GitHub-triggered update crash-looped the service with no log at all, and separately deleted the real on-device `config.json`. Root-caused by reading `GitHubDownload()`'s exact sequence in `PackageManager.py`: it downloads the archive, extracts it to a temp directory, **renames the entire existing `/data/<packageName>` directory aside**, moves the freshly-extracted archive into its place, then **permanently `shutil.rmtree`s the renamed-aside old directory**. This is a full directory replacement, not a merge -- anything in the old directory that isn't tracked in git is gone, no exceptions, and it happens generically for every SetupHelper package, not something specific to this one that could be special-cased. Two consequences, both hit at once:
  - See "Vendored, Not a Submodule" below for why `ext/velib_python` broke.
  - See "Config Lives Outside the Package Directory" below for why `config.json` was lost, and where it lives now.

### Vendored, Not a Submodule (`ext/velib_python`)

Originally a git submodule (Phase 2) -- reasonable for local development (easy to track/update against upstream), but fatal for GitHub-based auto-update: GitHub's `archive/<branch>.tar.gz` endpoint -- the only download mechanism `PackageManager.py`'s `GitHubDownload()` supports, hardcoded, no alternative -- never includes submodule content. Confirmed directly, not assumed: downloaded the real archive from this repo and inspected it -- `ext/velib_python/` was present as an empty directory entry, none of its actual files. Every GitHub-triggered update would have silently shipped a broken `ext/velib_python`, crashing `tank_service.py`/`switch_service.py`'s `from vedbus import VeDbusService` before `publisher.py` ever reaches its own logging setup (which is why the first real occurrence of this produced a crash loop with no application log at all -- the traceback only ever reached runit's raw stdout capture).

Fixed (2026-08-25) by vendoring the exact same commit (`17bbcd4c632d3eda484cde611dc78bf8c2ba469f`) directly into the repo as regular tracked files -- `.gitmodules` removed, no more `git submodule` step needed at clone time. Trade-off: pulling a future upstream `velib_python` change now means manually re-copying its files and committing them, instead of `git submodule update --remote` -- accepted, since this project only ever pins to one known-good commit anyway and has never needed to move it.
...
- Battery voltage is not published in Phase 2 — it's PID-based (request/response), not broadcast, and Phase 2 is passive-only (no bus transmission at all yet, not even a read request). Deferred, low priority.
